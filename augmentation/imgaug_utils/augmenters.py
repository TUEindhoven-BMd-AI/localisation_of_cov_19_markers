...
import itertools
from augmentation.imgaug_utils import imgaug as ia
            ...
            if self.random_order:
                ...
                if hasattr(self, 'invert'):
                    if self.invert:
                        ...
                        for augmenter in auglist[::-1]:
                             
                            augmenter.invert = self.invert
...
class Sometimes(Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        if hooks.is_propagating(images, augmenter=self, parents=parents, default=True):
            ...
            if hasattr(self, 'invert'):
                if self.invert:
                    self.then_list.invert = True
                    self.else_list.invert = True
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        for i in xrange(nb_images):
            ...
            if per_channel == 1:
                ...
                result[i] = image.astype(np.float)
            else:
...
class AddElementwise(Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        for i in xrange(nb_images):
            ...
            result[i] = after_add.astype(np.float)
        return result
...
class Multiply(Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        for i in xrange(nb_images):
            ...
            im_max_orig = np.max(image)
            im_min_orig = np.min(image)
            ...
            if per_channel == 1:
                ...
                for c, sample in enumerate(samples):
                    assert sample >= 0
                    image[..., c] *= sample
                np.clip(image, im_min_orig, im_max_orig, out=image)
                result[i] = image.astype(np.float)
            else:
...
class MultiplyElementwise(Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        for i in xrange(nb_images):
            ...
            np.clip(after_multiply, im_min_orig, im_max_orig, out=after_multiply)
            result[i] = after_multiply.astype(np.float)
        return result
        ...
        for i in xrange(nb_images):
            ...
            if per_channel:
                ...
                for c, alpha in enumerate(alphas):
                    image[..., c] = alpha * (image[..., c] - (np.max(image)-np.min(image))/2) + (np.max(image)-np.min(image))/2
            else:
                alpha = self.alpha.draw_sample(random_state=rs_image)
                vmin = np.min(image)
                vmax = np.max(image)
                image = alpha * (image - (vmin+vmax)/2) + (vmin+vmax)/2
                np.clip(image, vmin, vmax, out=image)
            result[i] = image.astype(np.float)
        return result
...
class Affine(Augmenter):
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        if hasattr(self, 'invert'):
            if self.invert:
                scale_samples = (1./scale_samples[0],1./scale_samples[1])
                translate_samples = (-translate_samples[0],-translate_samples[1])
                rotate_samples = -rotate_samples
        ...
        for i in xrange(nb_images):
            ...
            if scale_x != 1.0 or scale_y != 1.0 or translate_x_px != 0 or translate_y_px != 0 or rotate != 0 or shear != 0:
                ...
                minv = np.min(result[i])
                maxv = np.max(result[i])

                if (minv != maxv): #Only perform transformation on the targets it the target does not contain the same values.
                    input_i = (result[i]-minv)/(maxv-minv)
                    output_i = tf.warp(
                        np.clip(input_i,-1,1),
                        matrix.inverse,
                        order=0,#order,
                    ...
                    result[i] = output_i*(maxv-minv)+minv
                

        if isinstance(images, list):
            result = [image.astype(np.float, copy=False) for image in result]
        else:
            result = result.astype(np.float, copy=False)
    ...
    def _augment_keypoints(self, keypoints_on_images, random_state, parents, hooks):
        ...
        for i, keypoints_on_image in enumerate(keypoints_on_images):
            ...
            if scale_x != 1.0 or scale_y != 1.0 or translate_x_px != 0 or translate_y_px != 0 or rotate != 0 or shear != 0:
                ...
                result.append(ia.KeypointsOnImage.from_coords_array(np.around(coords_aug).astype(np.float), shape=keypoints_on_image.shape))
            else:
